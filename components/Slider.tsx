  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-2xl bg-black ${heightClass}`}>
      ...
      >
        {data.map((s, i) => (
          <div key={i} className="relative shrink-0 basis-full h-full">
            {/* 이미지가 슬라이더 박스를 꽉 채우도록 object-cover (비율 다른 경우 가장자리 크롭) */}
            <Image
              src={s.src}
              alt={`slide-${i + 1}`}
              fill
            ...
            {/* 오버레이/텍스트 (옵션) */}
            {withOverlayText && i === idx && (headline || subline) && (
              // 버튼 클릭 방해하지 않도록 pointer-events-none
                ...
                <div className="ml-auto text-right max-w-2xl sm:max-w-3xl md:max-w-4xl">
                  {headline && (
                    <h1
                      ...
                      style={{ wordBreak: "keep-all" }}  // Tailwind에 break-keep 없을 때 대비
                    >
                      {headline}
                    </h1>
                  )}
      ...
      <button aria-label="previous" onClick={prev}
        className="absolute z-20 left-3 top-1/2 -translate-y-1/2 rounded-full bg-white/80 hover:bg-white p-2 shadow">‹</button>
      <button aria-label="next" onClick={next}
        className="absolute z-20 right-3 top-1/2 -translate-y-1/2 rounded-full bg-white/80 hover:bg-white p-2 shadow">›</button>
